...
android_sdk=${ANDROID_HOME:-${ANDROID_SDK_ROOT:-/Users/iriver/.local/android-sdk}}
...
if find "$project_root" -path '*/build/outputs/*' -type f \( -name '*release*.apk' -o -name '*.aab' \) | grep -q .; then
  echo "release APK/AAB artifact exists" >&2
  exit 1
fi
