...
- [x] 공식 `rust-v0.147.0` ClientRequest schema에는 별도 `initialized` client notification이 없고 `initialize` request만 정의되어 있다. 따라서 지원되지 않는 notification을 추측해 보내지 않고, `initialize` response의 필수 platform/home fields 검증을 handshake 완료 조건으로 고정했다.
