...
- process 시작 직후 `initialize`와 initialized handshake가 완료돼야 `READY`로 승격한다.
...
- [ ] Phase 1 Alpine Runtime·Workspace 이식
- [ ] Phase 2 공식 Codex CLI artifact pack
- [ ] Phase 3 app-server supervisor·JSON-RPC adapter
- [ ] Phase 4 Gateway management API·chat stream
- [ ] Phase 5 Android Runtime bridge·Gateway client
...
- [ ] runtime/workspace 8개 모듈을 source map 기준으로 이식한다.
- [ ] rootfs, PRoot, loader, SBOM artifact와 기존 checksum 검증을 함께 이식한다.
- [ ] 기존 package namespace를 우선 유지해 불필요한 대규모 rename을 피한다.
- [ ] 신규 앱에 Runtime install/start/stop/status controller를 조립한다.
- [ ] app-private workspace와 Codex 전용 home/staging 경로를 정의한다.
- [ ] terminal/package UI는 화면에서 노출하지 않고 내부 smoke 실행 API만 유지한다.
- [ ] 기존 앱과 신규 앱의 파일·port·notification ID가 충돌하지 않도록 분리한다.

### 자체 테스트

- [ ] runtime/workspace JVM unit test PASS
- [ ] `:alpine-runtime-android:assembleDebug :app:assembleDebug` PASS
- [ ] bundled artifact SHA-256·ELF alignment verifier PASS
- [ ] Samsung에서 신규 앱 Runtime 설치·시작·상태·종료 PASS
- [ ] 기존 `dev.alpine.integrated.debug`와 신규 앱이 동시 설치됨
- [ ] 한 앱 Runtime start/stop이 다른 앱 process/files에 영향을 주지 않음
...
- [ ] 신규 app-private Alpine에서 `uname -m`, Python 실행을 bounded smoke로 확인함

## Phase 2. 공식 Codex CLI artifact pack 구현

### 구현 태스크

- [ ] `codex-cli.lock.json`을 PoC 버전 `0.147.0`과 공식 arm64-musl asset으로 작성한다.
- [ ] local path/cache/official URL 우선순위의 debug prepare task를 구현한다.
- [ ] archive와 binary의 크기·SHA-256·ELF architecture를 검증한다.
- [ ] generated debug asset을 APK에 넣되 Git 추적 대상에서 제외한다.
- [ ] `CodexCliArtifactProvider`와 atomic staging/version directory를 구현한다.
- [ ] permission 설정, hash 재검증, `codex --version` smoke를 startup gate로 연결한다.
- [ ] 실패한 partial staging과 이전 버전 정리 정책을 구현한다.
- [ ] CLI license/notice와 source URL을 debug 앱 notice 문서에 기록한다.

### 자체 테스트

- [ ] 올바른 local archive에서 prepare 성공
- [ ] archive hash 1-bit 변경 시 prepare 실패
- [ ] binary hash/size/ELF target 불일치 시 build 실패
- [ ] network가 없어도 verified cache가 있으면 debug build 성공
- [ ] Samsung Alpine에서 staged binary가 `codex-cli 0.147.0`을 반환
- [ ] APK/source scan에서 untracked credential과 client ID가 없음
...
- [ ] 공식 CLI가 재현 가능한 lock과 checksum을 통해 신규 앱 Alpine에서 실행됨

## Phase 3. Python app-server supervisor와 JSON-RPC adapter

### 구현 태스크

- [ ] `codex_gateway/app_server/process.py`에 process lifecycle owner를 구현한다.
- [ ] `jsonl.py`에 strict UTF-8, line size, JSON object validation parser를 구현한다.
- [ ] `rpc.py`에 request ID, pending map, timeout, notification fan-out을 구현한다.
- [ ] `protocol.py`에 account/login/model/thread/turn typed adapter를 구현한다.
- [ ] `redaction.py`에 stderr/error 축약 정책을 구현한다.
- [ ] initialize handshake와 protocol version/capability capture를 구현한다.
- [ ] stop/crash/timeout 시 pending request와 active turn을 정확히 한 번 종료한다.
- [ ] fake app-server executable fixture를 만들어 정상/오류/순서 뒤바뀜을 재현한다.

### 자체 테스트

- [ ] split/coalesced JSONL, CRLF, EOF, invalid UTF-8, oversized line
- [ ] out-of-order response, unknown/duplicate ID, request timeout
- [ ] stderr flood가 memory 상한을 넘지 않고 credential-like text가 redacted됨
- [ ] initialize success/failure/timeout/process exit
- [ ] active turn 중 crash가 자동 replay 없이 한 번의 terminal error로 종료됨
- [ ] Samsung의 실제 CLI `initialize`, `account/read` smoke PASS
...
- [ ] fake와 실제 CLI에서 supervisor가 `READY` 또는 stable failure로 결정적으로 수렴함

## Phase 4. Gateway management API와 chat stream 변환

### 구현 태스크

- [ ] 기존 Python Gateway를 fork해 direct Provider/config/API key 계층을 제거한다.
- [ ] `/healthz`에 runtime/gateway/codex protocol check를 추가한다.
- [ ] account/read, device login start/status/cancel, logout endpoint를 구현한다.
- [ ] `model/list` pagination, normalize, deduplicate, bounded cache를 구현한다.
- [ ] conversation↔thread와 gateway request↔turn registry를 구현한다.
- [ ] thread start/resume과 turn start/interrupt를 구현한다.
- [ ] agent delta/completion/error를 기존 SSE shape로 변환한다.
- [ ] disconnect, timeout, process crash 시 interrupt/cleanup을 idempotent하게 처리한다.
- [ ] active login/turn 1개 제한과 HTTP conflict 응답을 구현한다.
- [ ] gateway bundle build와 checksum manifest를 구현한다.

### 자체 테스트

- [ ] 미인증 account/model/chat의 status/error contract
- [ ] login start→pending→completed, cancel, expiry, duplicate start
- [ ] model pagination/duplicate/empty/malformed response
- [ ] first conversation thread/start, existing conversation thread/resume
- [ ] delta 0개/1개/다수, completed, failed, malformed notification
- [ ] client disconnect와 explicit interrupt가 turn/interrupt를 최대 1회 호출
- [ ] resume 실패 후 새 thread 자동 생성·prompt replay가 없음
- [ ] request/message/stream size limit과 loopback-only bind PASS
- [ ] API key/client ID/base URL config field가 package와 bundle에 없음
...
- [ ] fake app-server를 통한 login/model/1-turn/stop Gateway integration test PASS

## Phase 5. Android Runtime bridge와 bounded Gateway client

### 구현 태스크

- [ ] `CodexRuntimeController`에 runtime→CLI staging→Gateway start 순서의 lifecycle을 구현한다.
- [ ] 상태를 `STOPPED/STARTING/RUNNING/STOPPING/FAILED`로 직렬화한다.
- [ ] `CodexGatewayClient`에 health/account/login/model/chat/interrupt API를 구현한다.
- [ ] URL은 exact loopback scheme/host/port만 허용한다.
- [ ] JSON/SSE request/response/event/total size와 timeout을 제한한다.
- [ ] Android coroutine cancellation을 HTTP disconnect와 Gateway interrupt로 연결한다.
- [ ] foreground/background 전환과 app process recreation 뒤 health 재결합을 구현한다.
- [ ] direct Provider backend 없는 `CodexGatewayChatBackend`를 routing contract에 연결한다.

### 자체 테스트

- [ ] invalid endpoint, oversized body, malformed JSON/SSE, strict UTF-8
- [ ] login/account/model response mapping과 stable error mapping
- [ ] SSE start/delta/done/error/[DONE] state transition
- [ ] Stop 1회가 HTTP cancel과 interrupt 최대 1회를 유발
- [ ] runtime start 중복 tap과 start/stop race가 단일 stable state로 수렴
- [ ] active turn 이후 자동 backend fallback·재전송 없음
...
- [ ] Android fake Gateway test에서 lifecycle, OAuth 상태, model, 1턴 stream, Stop이 모두 PASS
