...
- 참조 프로젝트 README: `$ALPINE_REFERENCE_REPO/README.md`
- 참조 프로젝트 핸드오프: `$ALPINE_REFERENCE_REPO/HANDOFF.md`
- OAuth client ID·CLI proxy 논의: `$ALPINE_REFERENCE_REPO/dev-plan/handoff_20260811_111816_oauth_clientid_discussion.md`
- Provider OAuth·inference 경계: `$ALPINE_REFERENCE_REPO/docs/provider-oauth-adapters.md`
- Samsung 메뉴별 실동작 검증: `$ALPINE_REFERENCE_REPO/dev-plan/implement_20260811_102900.md`
...
| POST | `/internal/codex/login/active/cancel` | cancelled only | `no_active_login` |
...
- [x] Phase 6 Device OAuth·모델·채팅 UI
- [x] Phase 7 자동 회귀·artifact 안전 검사
...
- [x] 앱 시작 시 Runtime/Gateway/account/model 상태를 순서대로 로드한다.
- [x] Runtime 미설치·중지·시작 실패 상태 sheet와 복구 action을 구현한다.
- [x] Device Code 로그인 sheet에 browser 열기, code 복사, 상태 확인, 취소를 구현한다.
- [x] authenticated 뒤 모델 목록을 동적으로 표시하고 기본 모델을 선택한다.
- [x] 대화 목록/새 대화/암호화 저장과 Codex thread binding을 연결한다.
- [x] 채팅 입력, 1회 전송, stream 표시, Stop, stable error UI를 연결한다.
- [x] 상단 action을 상태/overflow 두 개로 제한해 대화 viewport를 최대화한다.
- [x] IME와 navigation bar inset 위에 입력창을 고정한다.
- [x] 모델 선택, 로그인, Runtime 상태에 stable semantics/test tag를 부여한다.
- [x] logout은 active turn이 없을 때 확인 dialog 뒤 실행한다.

### 자체 테스트

- [x] 미설치→설치→시작→로그인 필요→로그인 중→ready 상태별 UI
- [x] login 중 duplicate tap 차단, cancel/expiry/error 복구
- [x] model 0/1/다수/현재 모델 제거 상황
- [x] IME open, gesture/3-button navigation, landscape에서 composer 가시성
- [x] streaming 중 model/logout/runtime restart 비활성화
- [x] Stop 뒤 중복 assistant message·중복 완료 없음
- [x] process recreation 뒤 credential을 읽지 않고 account state를 복구

### 이슈 및 수정

- [x] AndroidKeyStore가 randomized-encryption key에 caller-supplied GCM IV를 거부해 대화 저장이 실패했다. 암호화 시 Keystore 생성 IV를 저장하고 복호화 시에만 제공하도록 수정했으며 Samsung encrypted-store test로 재검증했다.
- [x] 로그인 시작 비동기 요청 중 빠른 연타가 두 번째 Device Code 요청을 만들 수 있었다. ViewModel in-flight guard와 disabled UI action으로 단일 요청만 허용하도록 수정했다.
- [x] 재시작 직후 대화 목록 UI가 디스크 write 성공보다 먼저 갱신될 수 있었다. 저장 성공 뒤에 목록을 노출하도록 순서를 고정하고 Samsung process-recreation fake test로 재검증했다.
- [x] 앱 재진입 뒤 gateway에 이전 Device Code login만 남으면 새 시작 요청이 `login_already_active`로 막혔다. opaque login ID와 Device Code를 노출하지 않는 active-cancel endpoint와 사용자 명시 취소 UI를 추가했고, 자동 재시작 없이 Python 21 tests, debug Kotlin build, Samsung fake instrumentation 8 tests로 검증했다.

### 완료 조건

- [x] instrumentation fake backend에서 로그인부터 1턴/Stop/로그아웃까지 메뉴별 테스트 PASS (Samsung `SM-S931N`, 6 tests)

### 이슈 및 수정 결과

- [x] debug Compose test host가 test APK process에서 실행되어 Compose root를 찾지 못했다. 빈 host를 debug app 변형으로 옮기고 landscape config change를 처리해 target app process에서 실행되게 수정했다.
- [x] Device Code sheet의 취소 action이 landscape에서 보이지 않을 수 있었다. 본문을 스크롤 가능하게 하고 취소를 안내문 바로 아래로 이동했으며 Samsung touch instrumentation으로 callback을 재검증했다.

## Phase 7. 자동 회귀·artifact 안전 검사

### 구현 태스크

- [x] Python unit/integration suite를 단일 명령으로 구성한다.
- [x] Android JVM unit, instrumentation APK, lint, debug APK build gate를 구성한다.
- [x] pinned CLI protocol fixture와 current adapter method 목록을 검사한다.
- [x] source/APK에서 client ID, CLI fingerprint, API key, private key, Provider direct endpoint를 검사한다.
- [x] release signing/store key 참조와 release artifact 생성 경로 부재를 검사한다.
- [x] generated CLI binary/rootfs/auth/log가 Git tracked 되지 않았는지 검사한다.
- [x] dependency/license/SBOM 목록을 생성하고 debug artifact에 포함한다.
- [x] `scripts/verify-debug-milestone.*`에 전체 로컬 gate 순서를 고정한다.

### 자체 테스트

- [x] Python unit/integration 전체 PASS (20 tests)
- [x] Android unit/lint/assembleDebug/assembleDebugAndroidTest 전체 PASS
- [x] fake Gateway instrumentation 전체 PASS (Samsung `SM-S931N`, 6 tests)
- [x] secret/OAuth registration/artifact clean-room scan PASS
- [x] checksum/SBOM/reference source map verifier PASS
- [x] `git diff --check` PASS

### 이슈 및 수정

- [x] macOS 기본 Python 3.9에는 `tomllib`가 없어 inventory generator가 실패했다. 외부 의존성 없이 고정 version catalog 필드만 파싱하도록 수정하고 전체 verifier를 재실행했다.
- [x] 첫 milestone script의 집계 `test` task가 AGP release unit-test 변형까지 구성했다. release APK/AAB는 생성되지 않았고, 모든 Android test task를 explicit `testDebugUnitTest`로 고정한 debug-only gate를 재실행했다.
- [x] `lintDebug`가 이식 Runtime 모듈의 debug AndroidTest classpath를 해석할 때 격리 cache에 없는 AndroidX test 1.3.0/runner 1.7.0에서 중단됐다. 해당 테스트 의존성만 준비한 뒤 offline gate를 통과했다.
- [x] debug asset source set을 쓰는 AGP lint model/analysis 태스크가 CLI·Gateway·audit asset producer를 암시적으로 읽었다. 모든 이름의 debug lint consumer를 명시 선행 작업으로 연결하고 `lintDebug`를 통과했다.
- [x] APK 문자열 검사가 공식 고정 CLI binary 내부 문자열을 앱 구현물로 오인했다. 정확한 asset path와 lock의 size/SHA-256을 검증한 그 단일 binary만 byte-pattern 검사에서 제외하고 나머지 APK는 계속 검사하도록 수정했다.

### 완료 조건

- [x] 실제 계정 없이 재현 가능한 모든 로컬 gate가 한 번의 명령으로 PASS
...
- [x] E2E-IN-01: 신규 debug APK와 기존 reference debug 앱의 공존을 redacted evidence에 기록했다.
- [x] E2E-RT-01: Samsung에서 Alpine Runtime과 loopback Gateway가 모두 `RUNNING`임을 확인했다.
- [x] E2E-CLI-01: pinned CLI version과 `initialize → account/read` 무인증 smoke가 closed success state로 통과했다.
- [x] E2E-OA-01: 실제 Device Code challenge를 1회 시작하고 공식 브라우저 승인 페이지를 열었다. 앱 code-copy → 브라우저 paste/submit을 1회 시도했으며 code/URL은 읽거나 기록하지 않았다.
- [x] E2E-OA-02: 사용자 브라우저 승인 뒤 loopback `account`가 `authenticated: true`만 반환함을 확인했다. 계정 식별 정보와 auth 원문은 읽거나 기록하지 않았다.
- [ ] E2E-MD-01 이후: 인증 뒤 동적 모델·실제 stream chat·Stop·lifecycle·logout을 순서대로 검증한다.

### 구현 태스크

- [x] 실기기 사전 상태와 두 앱 package/version을 redacted evidence로 기록한다.
...
- [x] 실제 `MainActivity`는 기본 `AndroidViewModelFactory`가 `Application` 단일 생성자를 찾지만, `CodexChatViewModel`은 Kotlin default parameter만 가진 다중 인자 생성자여서 시작 시 crash했다. `@JvmOverloads`로 `Application` 생성자를 노출하고 reflection unit test·Samsung resumed Activity로 재검증했다.
