...
문서 상태: `IN_PROGRESS_PHASE_9`

최종 갱신: `2026-08-14 KST` — 현재 작업 트리 재분석과 host 회귀 테스트 결과를 반영했다.

구현 승인 상태: `GRANTED` — `2026-08-12 14:07:50 KST`에 첨부된 장기 Goal 프롬프트로 G1~G7과 Phase 0~9 구현이 승인됐다. Grok OAuth 시작은 `2026-08-13 KST`에 승인됐지만 host 보정 build의 로그인 action과 외부 브라우저 승인은 아직 완료되지 않았다. Grok logout과 실제 Codex 유료 turn은 실행 직전에 각각 별도 승인을 받는다.
...
- [Samsung Grok secure-debug E2E evidence](/Volumes/ExternalSSD/projects_8/alpine-codex-cli-client/docs/samsung-grok-secure-debug-e2e.md)
- [Grok Phase 9 handoff](/Volumes/ExternalSSD/projects_8/alpine-codex-cli-client/docs/grok-phase9-handoff.md)
- [Reference source map](/Volumes/ExternalSSD/projects_8/alpine-codex-cli-client/docs/reference-source-map.md)
- [Phase 9 잔여 작업 Goal 프롬프트](/Volumes/ExternalSSD/projects_8/alpine-codex-cli-client/dev-plan/goal_prompt_20260813_101420.md)
...
## 2026-08-14 최신 기준선과 재개 조건

이 표는 과거 Phase별 PASS 기록을 지우지 않고, 현재 작업 트리에서 다음 작업을 시작하기 위한
최신 기준선만 고정한다. 과거 secure-debug milestone PASS는 당시 소스의 증거이며 현재 최종
gate PASS로 재사용하지 않는다.

| 항목 | 현재 확인 결과 | 다음 판정 |
|---|---|---|
| Git | `codex/implement-grok-agent @ 007a290`; tracked 수정 34개와 untracked 7개가 Phase 9·theme·handoff 누적 진행분으로 남아 있음 | reset·stash·rebase·추가 pull 없이 보존 |
| Python 전체 회귀 | 현재 103/103 PASS; post-initialize barrier 및 Grok account-host 회귀 포함 | credential-free gate 완료 |
| 실패 테스트 | `test_bad_initialize_unknown_and_duplicate_id_fail_closed` | 수정 후 focused 7/7 PASS |
| race 재현 | 변경 전 `duplicate_id` 20회 중 2회 READY; 변경 후 100/100 `FAILED`, READY 0 | 결정적 fail-closed 확인 |
| Kotlin/Android gate | 전체 JVM/Android unit, `lintDebug`, `lintSecureDebug`, debug/test/secure APK build PASS; Samsung fake UI 6/6 PASS | 실제 계정 gate와 분리 유지 |
| diff 형식 | `git diff --check` PASS | 로컬 handoff commit 직전 재실행 |
| 현재 secure gate | Python 103 tests와 전체 APK/artifact/protocol/security gate PASS | host/UI 보정 source 검증 완료 |
| Samsung 기준선 | secure APK `161137250` bytes, SHA-256 `1a396b48ad1035250f0792541079689e9c4f76006436f0eb8495044d7c18a7f1`; version code 2, non-debuggable, 데이터 보존 설치 | app/PRoot/Python/Grok/Codex `1/1/1/1/0`, `DEVICE OAUTH` 준비 |
| 민감 action | host 보정 뒤 OAuth/model/prompt/Stop/logout dispatch 0 | 다음 사용자 click 뒤 승인·증거 gate 순서 유지 |

Codex app-server JSON-RPC의 initialize 중복 응답 경쟁 조건은 Phase 9A에서 닫혔다. 현재 실기기
진행 지점은 host 보정 build의 `Grok 로그인` 사용자 click과 공식 브라우저 승인이다. 입력 직전
target focus를 다시 확인하며 관련 없는 앱에는 input·force-stop·disable·uninstall·clear-data를
수행하지 않는다.
...
| Codex initialize 안정화 | `codex_gateway/app_server/process.py`, `rpc.py`, `tests/test_app_server_supervisor.py`, `tests/fake_app_server.py` | 중복·unknown initialize 응답이 외부 READY로 관찰되기 전 generation fail-closed |
...
- [x] Phase 8. 보안·자동 회귀·artifact gate 완료
- [x] Phase 9A. 현재 회귀 기준선 안정화 완료
- [ ] Phase 9B. Samsung 실제 OAuth·model·1턴 완료
- [ ] Phase 9C. Stop·lifecycle·Agent 전환·logout 완료
- [x] Phase 9D. 최종 secure gate·문서·Git handoff 완료
...
- [x] 인접한 정상 initialize 응답과 duplicate response를 모두 읽기 전에 supervisor가 READY를 외부에 확정하지 않는다.
- [x] duplicate initialize fixture를 반복 실행해 `start()` 성공 0회와 최종 `FAILED`를 확인한다.
- [x] 과거 milestone PASS 기록과 현재 작업 트리의 PASS/FAIL 결과를 문서에서 구분한다.
- [x] E2E·handoff·계획 문서의 APK identity와 테스트 개수를 최종 실측값 하나로 통일한다.
...
- [x] 보안 계획의 secure variant와 authenticated Gateway를 Grok endpoint에도 적용한다.
- [x] Grok HOME/profile/config permission과 symlink 검사를 artifact gate에 추가한다.
- [x] source/APK/env/profile에서 API key path, OAuth client ID, CLI fingerprint, direct xAI endpoint, auto-update를 검사한다.
- [x] ACP forbidden-method negative fixture를 milestone verifier에 추가한다.
- [x] Grok binary version·size·hash·ELF·Git exclusion을 검증한다.
- [x] Python/Kotlin/unit/integration/instrumentation test를 debug-only host gate와 격리된 Samsung lab gate로 묶는다.
- [x] Codex 기존 로그인·model·1-turn·Stop·logout fake regression을 같은 gate에서 실행한다.
- [x] logcat/evidence scanner에 Grok URL/code/token/email/prompt/response 패턴을 추가한다.

### 자체 테스트

- [x] unsigned/replay/browser request가 Codex/Grok 모든 endpoint에서 거부된다.
- [x] lab debuggable app에서는 실제 OAuth action이 차단된다.
- [x] secure app에는 instrumentation test package가 결합되지 않는다.
- [x] 전체 fake OAuth→model→1-turn→Stop→process-recreation state→logout이 두 Agent에서 통과한다.
- [x] API key/client ID/fingerprint/direct fallback/forbidden ACP method scan이 통과한다.
- [x] debug APK audit inventory와 lock/hash가 일치하고 release artifact가 없다.

### 이슈 및 수정

- [x] fresh Alpine에는 Python이 없어 기존 `Runtime 시작` 한 action이 Gateway 준비 전 단계에서 timeout 됐다. `Alpine 시작 → Gateway Python 준비 → Gateway 시작`을 사용자 명시 action으로 분리하고, 설치는 exact `apk add --simulate python3` 성공 뒤에만 수행하도록 수정했다.
- [x] Runtime-only 계측 뒤 같은 instrumentation process에서 첫 Compose Activity hierarchy가 간헐적으로 생성되지 않았다. credential-free Runtime, Agent workflow, Agent UI, Codex workflow/UI를 별도 process로 격리했고 각 그룹 전체 PASS를 확인했다.
- [x] 모델이 많은 BottomSheet가 partial expansion 상태에서 마지막 항목을 화면 밖에 남겼다. 모델 시트를 완전 확장 `LazyColumn`으로 바꾸고 첫 항목부터 마지막 항목까지 실제 스크롤·표시·잠금 테스트를 통과했다.
- [x] 기존 Codex fake Stop이 `start` event 뒤 stop latch를 생성해 즉시 Stop을 놓치는 test-only race가 있었다. latch를 start 전에 준비해 one-shot Stop 계약과 일치시키고 기존 Codex workflow 3/3을 재검증했다.
- [x] Phase 8 redacted 결과와 최종 secure APK identity/hash는 `docs/grok-phase8-security-evidence.md`에 기록했다. OAuth·계정·동적 model 내용·chat 원문은 관찰하거나 기록하지 않았다.

### 완료 조건

- [x] 실제 Grok OAuth를 시작해도 되는 보안·회귀 gate가 모두 통과했다.
- [x] Samsung E2E 실행 승인 요청에 필요한 redacted manifest가 준비됐다.
- [x] 다음 Phase 진행 가능
...
- 현재 작업 트리의 Python 전체 회귀가 98/98 PASS하지 않으면 실제 OAuth·model·prompt·Stop mutation을 시작하지 않는다.
- credential, URL/code, 계정 정보, prompt·response 원문을 캡처해야만 진행 가능한 경우 중단한다.

### Phase 9A. 현재 회귀 기준선 안정화

#### 목표

- Codex initialize 중복 응답 race를 제거하고 현재 작업 트리의 host gate를 다시 녹색으로 만든다.
- 과거 Phase 8 PASS 기록과 현재 재검증 결과를 구분해 Phase 9 실기기 mutation의 선행 조건을 고정한다.

#### 구현 태스크

- [x] duplicate initialize response가 첫 정상 response 직후 도착해도 `start()`가 READY를 반환하지 않도록 supervisor/RPC generation 경계를 수정한다.
- [x] unknown ID, duplicate ID, late response, process exit의 기존 fail-closed 의미와 stable error code를 보존한다.
- [x] production에서 임의 지연이나 fixture 전용 sleep에 의존하지 않는 결정적 완료 경계를 설계한다.
- [x] 변경 범위를 Codex app-server supervisor/RPC와 직접 회귀 fixture에 한정하고 Grok protocol을 함께 재설계하지 않는다.

#### 자체 테스트

- [x] `python3 -m unittest tests.test_app_server_supervisor -v` PASS
- [x] `duplicate_id` direct stress를 최소 100회 실행해 `start()` READY 반환 0회를 확인한다.
- [x] `python3 -m unittest discover -v`가 현재 98/98 PASS한다.
- [x] 변경 전 기준선으로 `./gradlew testDebugUnitTest`가 176 tasks, BUILD SUCCESSFUL임을 확인했다.
- [x] 변경 후 `./gradlew testDebugUnitTest`를 다시 PASS한다.
- [x] `git diff --check`를 PASS한다.

#### 이슈 및 수정

- [x] `2026-08-14 KST` 전체 Python 회귀에서 98개 중 97개가 통과하고
  `test_bad_initialize_unknown_and_duplicate_id_fail_closed` 1개가 실패했다.
- [x] 관리형 sandbox의 loopback bind 때문에 최초 실행 21개가 환경 오류였지만, 승인된 local
  loopback 환경에서 다시 실행해 환경 오류를 제거하고 실제 실패 1개를 분리했다.
- [x] direct stress에서 `unknown_id`는 20/20 fail-closed였고 `duplicate_id`는 20회 중 2회
  `start()`가 READY를 반환해 reader scheduling 경쟁 조건임을 확인했다.
- [x] `initialize` 성공과 필수 `initialized` notification 뒤 동일 전체 timeout 안에서 공식
  `account/read`를 readiness barrier로 요청하도록 수정했다. child stdout의 순서 보장 때문에
  이미 출력된 duplicate/unknown initialize response는 barrier response보다 먼저 처리되고,
  supervisor는 외부 READY를 공개하기 전에 generation을 `FAILED`로 종료한다. 신규 endpoint,
  임의 sleep, Grok protocol 변경은 없다. focused 7/7, duplicate stress 100/100 `FAILED`,
  Python 전체 98/98와 Gradle debug unit 176 tasks를 통과했다.

#### 완료 조건

- [x] Python 전체 회귀 98/98와 duplicate stress 100/100 fail-closed가 통과한다.
- [x] Kotlin/Android unit와 diff 형식 검사가 통과한다.
- [x] Phase 9B Samsung OAuth 진행 가능

### Phase 9B. 공식 Grok OAuth·동적 모델·실제 1턴

#### 목표

- 최신 exact Samsung 기준선에서 공식 Grok Device OAuth와 실제 스트리밍 1턴을 원문 없는 증거로 검증한다.

#### 구현 태스크

- [x] APK update mutation 직전 exact Samsung model/ABI/API, target package/version/non-debuggable/signing을 다시 확인하고, launcher focus에서는 좌표 입력을 보내지 않는다.
- [x] OAuth action 직전 target app 또는 target-owned popup focus를 다시 확인한다.
- [x] 설치본과 새 secure APK의 size/SHA를 실측해 현재 source 기준 최신 값 하나로 고정한다.
- [x] Runtime/Gateway RUNNING, Grok child 1, Codex child 0을 content-free count로 확인한다.
- [x] 승인된 Grok login action을 정확히 1회 실행했다. 브라우저 handoff는 관찰되지 않았고 자동
  OAuth retry는 0회이며, 보정 build에서 두 번째 login action은 실행하지 않았다.
- [x] 사용자가 URL 준비 polling 보정 build에서 login action을 직접 다시 실행했다. 브라우저
  handoff 대신 `grok_login_challenge_invalid`가 표시됐고 추가 자동 retry는 없었다.
- [ ] accounts host 보정 build에서 사용자가 login action을 다시 실행한 뒤 공식 xAI 브라우저
  승인을 완료한다.
- [ ] 승인 완료 뒤 `authenticated=true` Boolean만 확인한다.
- [ ] 실제 dynamic catalog count가 1 이상이고 선택 ID가 catalog에 존재함만 기록한다.
- [ ] 첫 session에서 tool/subagent/MCP/filesystem/terminal count가 모두 0인지 확인하고, 하나라도 0이 아니면 prompt 전에 중단한다.
- [ ] 비민감 테스트 prompt를 정확히 1회 전송하고 원문 없이 user/assistant/terminal 및 dispatch count만 판정한다.

#### 자체 테스트

- [x] 첫 시도 login action 1, OAuth 자동 retry 0, 민감 URL/code/account 원문 기록 0
- [ ] model catalog count 1 이상, hardcoded fallback 0, selected-ID match true
- [ ] user node 1, assistant node 1, terminal event 1
- [ ] Android/Gateway dispatch 각 1, prompt replay 0, Agent fallback 0
- [ ] `AgentTurnAudit` 한 줄이 `scripts/verify-agent-turn-audit.py chat`을 통과한다.

#### 현재 진행 기록

- [x] `2026-08-14 KST` 현재 source로 `verify-secure-debug-milestone.sh`를 재실행해 Python
  98 tests, Gradle 516 tasks와 Codex/Grok protocol·artifact·APK·민감정보 gate 전체 PASS를
  확인했다.
- [x] 새 secure APK는 `161108302` bytes, SHA-256
  `d3e51532d979839448c0fe6e2b0987a3d1fd03bd2169abfad30c30735012d500`이며 package/version/
  non-debuggable/debug certificate가 기존 설치본과 일치했다. exact Samsung에 `adb install -r`로
  update-install했고 uninstall·clear-data·다른 기기 mutation은 0이다.
- [x] update-install 뒤 앱·Runtime child가 정지된 것을 확인하고 target app에서 Alpine session을
  정확히 1회 시작해 `RUNNING / RUNTIME_OPERATION_COMPLETED`에 도달했다. Runtime session open은
  child를 상주시킨다는 뜻이 아니며 첫 execute 전 PRoot 0은 제품 실행 모델과 일치한다.
- [x] `Gateway Python 준비` 직전 관련 없는 `com.stt.benchmark`가 target 전면화 직후에도 focus를
  재탈취했다. 동일 기기 shell의 target-focus guard가 종료 코드 42로 input을 차단했고 30초
  read-only 대기 뒤에도 process가 유지됐다. 해당 앱을 변경하지 않았으며 현재 build의
  Python/Gateway/OAuth/model/prompt/Stop/logout action은 모두 0으로 유지했다. 이후 관련 process가
  사라지고 target focus를 다시 확인한 뒤 Runtime/Gateway 복구를 계속했다.
- [x] `2026-08-14 KST` Grok child가 standalone 제품 smoke에서는 `READY`지만 Gateway HTTP
  worker가 종료되면 `grok_acp_process_lost`가 되는 PRoot lifecycle 차이를 분리했다. Gateway
  serve 전 생성되는 장수 `grok-acp-spawn-owner`가 고정 argv/env/cwd/closed-fd launch를 소유하게
  수정했고, Samsung에서 app/PRoot/Python/Grok/Codex `1/1/1/1/0` 기준선과 로그인 준비 상태를
  확인했다. 선택 backend process는 여전히 Grok 1개뿐이다.
- [x] production supervisor READY 공개 전에 0.5초 no-exit 구간과 ordered `_x.ai/auth/info`
  barrier를 추가했고, 너무 이른 auth-info를 거부하는 fixture로 회귀를 고정했다.
- [x] 첫 OAuth login action을 target focus와 `1/1/1/1/0` 기준선에서 정확히 1회 실행했다.
  8초까지 외부 browser focus는 관찰되지 않았고 민감 UI dump/screenshot/raw log는 시작 시점부터
  사용하지 않았다. authenticate 직후 `get_url`을 1회만 조회해 not-ready를 실패 처리하는 race를
  원인으로 확인했다.
- [x] authenticate는 사용자 action당 1회로 유지하고, exact not-ready 응답에 한해서만
  `_x.ai/auth/get_url`을 최대 15초 polling하도록 수정했다. invalid mode/URL/host와 timeout은
  fail-closed하고 동일 auth sequence를 cancel한다. focused 50/50, 전체 Python 102/102, Gradle
  516 tasks와 모든 secure milestone gate가 PASS했다. 전체 URL 준비 deadline을 각 ACP 요청에
  전달해 단일 호출도 남은 시간을 넘지 못하게 고정했다. 최신 APK `161109410` bytes, SHA-256
  `8f9ae09b510adad68535022bf22bfa8cc7b710891c51a9044c48204065760b66`를 데이터 보존
  update-install했고 OAuth 재시도 없이 `1/1/1/1/0` 기준선을 복구했다.
- [x] 사용자 재시도에서 `grok_login_challenge_invalid`가 재현됐다. 고정 upstream 1.0.0의
  `auth/config.rs`를 다시 대조해 `auth.x.ai`는 OAuth issuer이고 production accounts-app origin은
  `accounts.x.ai`임을 확인했다. Gateway와 Android가 issuer host만 허용해 정상 account URL을
  이중 차단하던 것이 원인이었다.
- [x] Gateway와 Android의 Grok URL 검증을 exact `auth.x.ai` + `accounts.x.ai`로 맞추고,
  subdomain/suffix lookalike·userinfo·비표준 port·fragment 차단은 유지했다. 로그인 클릭 직후
  `LOGIN STARTING`, progress indicator, `로그인 주소 요청 중`을 표시하도록 보완했다.
- [x] 보정 후 Python 103/103, Android unit/secureDebug assemble, Android 16 Samsung의
  credential-free `AgentChatSurfaceInstrumentedTest` 6/6이 PASS했다. APK `161137250` bytes,
  SHA-256 `1a396b48ad1035250f0792541079689e9c4f76006436f0eb8495044d7c18a7f1`를 데이터 보존
  update-install했고 target Activity 전면화를 확인했다. 실제 OAuth action은 자동 실행하지 않았다.
- [x] 전체 secure milestone의 JVM/Android unit, `lintDebug`, `lintSecureDebug`, debug/test/secure
  APK, Codex/Grok fixture·artifact·profile·ACP, clean-room·민감정보·source-map·manifest gate가
  모두 PASS했다. 설치 후 app-private Runtime/Gateway를 복구해 app/PRoot/Python/Grok/Codex
  `1/1/1/1/0`과 전면 `DEVICE OAUTH`/`Grok 로그인` 준비 상태를 확인했다.

#### 완료 조건

- [ ] official Grok Device OAuth 성공
- [ ] actual dynamic model 표시·선택 성공
- [ ] 실제 Grok 1턴 및 content-free audit 성공
- [ ] Phase 9C 진행 가능

### Phase 9C. Stop·lifecycle·Agent 전환·logout

#### 목표

- 별도 Stop 요청, 앱 lifecycle, Codex 무과금 회귀, Grok 재선택과 승인된 logout을 검증한다.

#### 구현 태스크

- [ ] 첫 chat과 분리된 새 요청을 1회 시작하고 Stop action을 최대 1회 실행한다.
- [ ] background→foreground 뒤 auth/model/conversation 유지와 중복 process 0을 확인한다.
- [ ] exact package 재확인 후 target package만 force-stop하고 재실행한다.
- [ ] 자동 prompt replay·자동 chat 없이 축약 상태가 복구되는지 확인한다.
- [ ] idle에서 Codex를 선택해 유료 turn 없이 account/model/readiness/conversation 보존을 확인한다.
- [ ] idle에서 Grok을 재선택해 인증·모델 축약 상태와 one-process invariant를 확인한다.
- [ ] active turn 0을 확인한 뒤 Grok logout 별도 승인을 요청하고, 승인 후 official logout을 정확히 1회 실행한다.
- [ ] Gateway와 Runtime을 정상 종료하고 PRoot/Python/Codex/Grok child 0을 확인한다.

#### 자체 테스트

- [ ] Stop audit: dispatch 1, cancel 최대 1, terminal 1, replay 0
- [ ] `scripts/verify-agent-turn-audit.py stop` PASS
- [ ] force-stop 후 자동 prompt replay 0, backend child agent별 최대 1
- [ ] Codex credential/conversation 불변, Grok 자동 fallback 0
- [ ] logout dispatch 1, Codex logout 0, Grok unauthenticated true
- [ ] Runtime 종료 후 관련 child process 0

#### 완료 조건

- [ ] Stop·lifecycle·Agent 전환·logout·cleanup 성공
- [ ] 민감정보 원문 기록 0
- [ ] Phase 9D 진행 가능

### Phase 9D. 최종 secure gate·문서·Git handoff

#### 목표

- 현재 소스와 실제 Samsung 결과를 하나의 재현 가능한 최종 기준선으로 고정한다.

#### 구현 태스크

- [x] `sh scripts/verify-secure-debug-milestone.sh`를 현재 소스로 전체 실행한다.
- [x] Python 103 tests와 전체 JVM/Android unit, lint, APK, artifact, protocol/security gate 결과를 실제 출력에서 기록한다.
- [x] Phase 9 체크박스와 최종 완료 정의를 현재 실행 결과에 맞춰 갱신한다.
- [x] `docs/samsung-grok-secure-debug-e2e.md`의 APK identity와 E2E 판정을 최신 실측값으로 통일한다.
- [x] README, architecture, security model, source map, runbook과 전용 Phase 9 handoff 중 실제 변경이 필요한 문서를 갱신한다. SBOM component 입력은 바뀌지 않아 본문 수정을 생략한다.
- [x] credential, OAuth URL/code, account, model 원문, prompt/response, APK/binary/capture가 tracked·untracked diff에 없는지 clean-room·sensitive evidence gate로 검사한다.
- [x] GPL-3.0 선언과 루트 license 파일 부재는 Phase 9 기능 범위 밖 후속 과제로 유지하고, 코드 정리를 확장하지 않는다.
- [x] `git diff --check`, branch, status, commit 대상 파일을 확인하고 `docs/grok-phase9-handoff.md`에 현재 dirty tree를 기록한다.
- [x] 검증 가능한 Phase 9 기능 통합 commit `7c0f88a`를 생성하고, 기존 no-push 제한은
  2026-08-14 사용자 명시 요청에 따라 `origin/main` fast-forward push로 전환한다.

#### 자체 테스트

- [x] 최종 secure-debug milestone 전체 PASS
- [x] 문서의 APK identity·테스트 개수·Phase 상태가 상호 일치
- [x] 민감 원문 및 금지 binary/artifact Git 포함 0
- [x] branch `codex/implement-grok-agent`, 기능 통합 commit `7c0f88a`, 최종 HEAD와 `origin/main` 일치

#### 완료 조건

- [x] 보안·artifact·문서·Git handoff 완료 정의 충족; 실제 OAuth·1턴 E2E는 Phase 9B/9C에 유지
- [ ] Phase 9 전체 완료
...
- [x] 실행 직전 연결 기기와 package identity를 다시 확인한다.
...
- [x] README, architecture, security model, source map, runbook, branch/commit 목록을 갱신한다. SBOM 입력은 변경 없음.
...
- [x] Phase 7의 dispatch/cancel/retry metric이 Python test API에만 있어 non-debuggable secure 앱의
  실제 turn을 원문 없이 판정할 수 없었다. 기존 인증된 terminal SSE에 bounded 숫자/enum
  `diagnostics`만 추가하고 Android strict parser와 content-free `AgentTurnAudit` 한 줄로 전달했다.
  새 endpoint, identifier, model, URL, 계정, message text는 추가하지 않았다.
- [x] 최초 authenticated session의 금지 프로필 event 0건 확인이 operator 관찰에만 의존했다.
  pinned upstream의 tool/subagent/MCP/filesystem/terminal wire family를 generation별로 집계하고,
  관찰 즉시 process를 fail-closed하며, 첫 prompt JSONL write와 같은 lock 안에서 5개 count가
  모두 0인지 검사하도록 수정했다.
- [x] 실제 turn audit를 사람이 복사해 해석하면 잘못된 retry/cancel/profile count를 통과시킬 수
  있었다. stdin의 fixed redacted line 1개만 허용하고 chat/Stop별 invariant와 G1 retry 범위를
  자동 판정하는 `verify-agent-turn-audit.py`와 negative fixture를 추가했다. verifier는 로그나
  원문 파일을 생성하지 않는다.
- [x] `2026-08-13 KST` 사용자의 `Grok OAuth 시작 승인` 후 첫 mutation 직전 exact target,
  Samsung `SM-S931N`, `arm64-v8a`, API 36, target package version code 2, non-debuggable,
  기존 설치본과 최종 secure APK의 debug certificate SHA-256 일치를 재확인했다.
- [x] 같은 debug certificate의 최종 secure APK를 `adb install -r`로 update-install했고
  uninstall·clear-data·release/store signing 사용 없이 version code 2, non-debuggable을 유지했다.
- [x] 연결된 Samsung의 관련 없는 `com.stt.benchmark.deviceTest`가 Alpine Agent Client를
  전면화한 직후 수 초 내에 반복적으로 focus를 가져가 Runtime UI 조작을 완료할 수
  없었다. 매 입력 전 target focused app을 확인해 다른 패키지에는 tap을 전달하지
  않았다. 재개 시점에 해당 process 0과 target focused app을 확인해 force-stop 없이
  경합이 해소됐고 Runtime UI를 계속할 수 있다.
- [x] update-install 후 비파괴 `Runtime 상태 확인`에서 실제 Alpine 상태가
  `NOT_INSTALLED`로 확인됐다. target app data는 보존되었지만 Runtime rootfs가 없으므로,
  앱 내 `Runtime 설치`를 선행하고 integrity/state 확인 후 원래 Alpine→Python→Gateway
  순서를 계속했다. 설치는 `READY / RUNTIME_OPERATION_COMPLETED`로 종료됐다.
- [x] 앱 UI에서 `Alpine 시작 → Gateway Python 준비 → Gateway 시작`을 각각 1회
  실행했고 `RUNNING / RUNTIME_SMOKE_PASSED / Gateway RUNNING`을 확인했다.
  관련 process는 app·PRoot·Python 각 1개였고, Agent UI가 authenticated transport를 통해
  Codex의 축약 `LOGIN_REQUIRED` 상태를 정상적으로 조회했다.
- [x] 첫 Grok 선택 직후 UI가 `MALFORMED_RESPONSE`로 종료되고 Grok child 0이었으며,
  명시적 `상태 다시 확인` 1회에서도 같은 결과가 재현됐다. Gateway/Runtime은
  계속 RUNNING이고 OAuth·prompt dispatch는 0이다. failed supervisor를 STOPPED로 회수한 뒤
  명시적 Codex→Grok 재선택을 1회 실행했으나 동일하게 실패해 지속 startup 결함으로 판정했다.
- [x] secure 앱에 raw stdout/stderr나 임의 command를 노출하지 않고 pinned Grok smoke의 exact
  content-free marker만 `READY`, staging, policy, version, process, initialize, invalid enum으로
  정규화하는 `Grok ACP 점검`을 추가했다. strict UTF-8·64-byte bound·stderr/timeout·exit-code
  fail-closed 단위 테스트를 추가했고, 첫 실행에서 이 모듈에 없는 `kotlin.test` import를 발견해
  기존 JUnit 4 계약으로 교정한 뒤 `:app:testDebugUnitTest`를 통과했다.
- [x] 첫 smoke는 initialize 응답의 protocol/auth method만 검사해 제품 supervisor의
  capability·session·동적 model-state strict parse와 달랐다. 진단을 실제
  `GrokAcpSupervisor.start()`와 READY 안정 구간으로 교체했고 Samsung에서 다시 `READY`를
  확인해 artifact·process·제품 initialize가 정상임을 판정했다.
- [x] failed backend에서 Android가 Agent catalog를 성공 경로에만 저장해 selector가
  비활성화됐고, selected Agent ID가 같으면 상태 확인이 복구 select를 보내지 않아 영구
  `MALFORMED_RESPONSE`가 됐다. 검증된 catalog를 readiness 실패 전에도 보존하고,
  `backendReady=false`면 같은 Agent를 명시적으로 select해 router가 failed generation을
  stop→start하도록 수정했다. ready same-Agent는 no-op이고 active login/turn은 그대로 차단된다.
  Samsung에서 catalog 복구 후 Codex child 1/Grok child 0 회수가 성공했으며 다른 Agent
  fallback, OAuth 또는 prompt dispatch는 없었다.
- [x] 최신 adapter/account 진단 secure APK를 Samsung에 데이터 보존 update-install하고
  `Grok ACP 점검`을 정확히 1회 실행한 결과 `ACCOUNT_FAILED`로 판정됐다. artifact·process·제품
  initialize·adapter lifecycle까지는 통과하고 OAuth 전 `x.ai/auth/info` account 축약 단계에서만
  실패한다. account payload나 raw stdout/stderr는 관찰하지 않았고 OAuth·model·prompt·Stop·logout
  dispatch는 모두 0이다.
- [x] 공식 Grok handler의 논리 extension 이름과 ACP 0.10.4의 JSON-RPC wire 이름을 동일하게
  취급한 것이 `ACCOUNT_FAILED`의 원인이었다. upstream `Cargo.lock` checksum과 일치하는 공식
  `agent-client-protocol 0.10.4` crate를 검증해 outbound extension에는 `_`가 하나 붙고 inbound는
  `_`를 제거한 뒤 handler로 전달됨을 확인했다. request/notification allowlist, adapter, fake ACP,
  fixture와 verifier를 exact `_x.ai/*` wire 계약으로 수정해 fake가 결함을 가리던 문제도 제거했다.
  focused Python 44 tests와 ACP verifier, unprefixed wire scan, 전체 secure debug milestone의 Python
  96 tests·Gradle 516 tasks·모든 보안 gate가 PASS했다. 수정 secure APK `161107678` bytes,
  SHA-256 `6bc19f4bfe08461c30ee665a3529a0cd60f698ba44a74ac0b7cb3e2f3e453ec0`를 exact Samsung에
  데이터 보존 update-install했고 package/version/non-debuggable/signing이 일치했다. 다만 외부
  `com.stt.benchmark.deviceTest`가 수 초마다 포커스를 재탈취해 target focus guard가 Python 준비와
  진단 입력을 차단했다. stale 좌표가 Runtime 종료에 해당해 Runtime은 안전하게 READY/STOPPED,
  child `app=1, PRoot/Python/Codex/Grok=0`으로 회수됐으며 OAuth·model·prompt·Stop·logout은 모두
  0이다. 관련 없는 앱 force-stop은 실행하지 않았다.
- [x] 관련 없는 앱을 변경하지 않고 target focus를 재확인한 뒤 Python 준비→Gateway 시작→Grok
  실제 backend를 복구했다. 최신 설치본의 app/PRoot/Python/Grok/Codex count는 `1/1/1/1/0`이다.
- [x] direct HTTP worker launch에서 Grok child가 worker return 뒤 유실되는 PRoot lifecycle 결함을
  Gateway 시작 전 생성되는 장수 spawn-owner로 수정했다. fixed command/environment, closed fd,
  process-wide umask 077, selected backend 1개 계약은 유지된다.
- [x] 첫 OAuth action은 1회였지만 browser handoff가 없었다. authenticate 시작 직후 아직 준비되지
  않은 `get_url` 응답을 즉시 실패 처리하던 race를 수정해 exact not-ready만 최대 15초 polling하고,
  authenticate 중복은 금지했다. 해당 보정 APK 설치 직후에는 재시도하지 않았다.
- [x] 후속 사용자 재시도는 `grok_login_challenge_invalid`로 끝났다. 공식 1.0.0 source의
  production accounts-app origin과 앱 검증 정책을 대조해 `accounts.x.ai` 누락을 확인했고,
  Gateway/Android exact host 정책과 클릭 진행 UI를 보정했다.

검증 기록: spawn-owner와 OAuth URL readiness polling 수정 후 전체 secure debug milestone PASS — Python 102 tests,
전체 Kotlin/JVM·Android unit, `lintDebug`, `lintSecureDebug`, lab debug APK·instrumentation APK,
non-debuggable secure debug APK, Codex fixture, clean-room, Grok artifact/profile/ACP, secure APK,
sensitive evidence, source map, Runtime manifest gate를 통과했다. 현재 Samsung 설치본은 수정
secure APK `161109410` bytes, SHA-256
`8f9ae09b510adad68535022bf22bfa8cc7b710891c51a9044c48204065760b66`, 기존 debug certificate SHA-256
`6bfd1a305ea7958397ca2d4c0a4917ea90b6b6fe75ea8b38b56001c05b4a1092`다. 이후 사용자 재시도로
accounts-host 누락이 확인됐고, host/UI 보정은 Python 103 tests와 Samsung fake UI 계측 6 tests를
통과했다. 현재 Samsung 설치본은 secure APK `161137250` bytes, SHA-256
`1a396b48ad1035250f0792541079689e9c4f76006436f0eb8495044d7c18a7f1`이다. 보정 뒤 실제 OAuth,
model request·prompt·Stop·logout은 자동 시작하지 않았다.

### 완료 조건

- [ ] Samsung에서 Grok CLI-owned OAuth와 실제 1턴, Stop, force-stop 복구, logout이 성공했다.
- [ ] Codex 기존 기능과 credential이 보존됐다.
- [x] 보안·artifact·문서·Git handoff 완료 정의가 충족됐다.
...
- [x] 현재 작업 트리의 Codex duplicate initialize race가 해결되고 Python 전체 회귀 98/98가 통과한다.
- [x] 공식 Grok CLI artifact가 version·size·SHA-256·ELF로 고정되고 binary는 Git에 없다.
- [x] Grok CLI는 app-private Alpine에서 auto-update·telemetry·subagent·API key auth가 비활성화된다.
- [x] Android/Python에 xAI OAuth client ID, API key 입력·저장·전달, direct Provider fallback이 없다.
- [x] Grok ACP method allowlist가 API key/token export와 임의 tool method를 차단한다.
- [x] Codex와 Grok HOME·credential·config·session/conversation이 분리된다.
- [x] Agent switch는 one active process/login/turn invariant를 유지한다.
...
- [x] application ID `dev.alpine.codexclient.debug`가 유지되고 표시 이름만 Agent 중립적으로 갱신된다.
- [x] secure variant·authenticated transport·민감 UI·permission gate가 신규 Grok 경로에도 적용된다.
- [x] credential·verification URL/code·계정 정보·prompt/response 원문이 로그·문서·캡처·Git에 없다.
- [x] release signing/store key/release APK/AAB를 사용하지 않았다.
- [x] 전체 debug-only build/unit/integration/instrumentation/adversarial/artifact gate가 통과했다.
- [x] redacted 문서와 branch/commit 목록으로 다른 개발자가 재현·인계할 수 있다.

## 잔여 리스크 / 후속 과제

다음 항목은 Phase 9 완료를 막는 선행 결함 또는 handoff 항목과 분리해 관리한다. Phase 9 도중
범위를 넓혀 함께 리팩터링하지 않는다.

- [ ] Agent-aware UI 전환 후 남은 Codex 전용 ViewModel/UI/client 호환 계층의 실제 참조를 조사하고 별도 정리 계획을 만든다.
- [ ] GitHub Actions 등 CI에서 host-only gate를 자동 실행하는 별도 계획을 만든다.
- [ ] README의 GPL-3.0 선언에 대응하는 루트 `LICENSE` 또는 `COPYING` 추가를 별도 라이선스 정리로 처리한다.
- [ ] 대형 `AgentChatViewModel`과 `GrokAgentAdapter` 책임 분리는 Phase 9 완료 후 별도 리팩터링 계획에서 다룬다.
- [ ] Codex/Grok 공식 CLI 업그레이드와 checksum 재검토 절차를 별도 유지보수 문서로 고정한다.
