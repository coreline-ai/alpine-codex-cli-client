...
- [ ] Phase 1. 공통 Agent 계약·Codex 회귀 fixture 완료
- [ ] Phase 2. Grok CLI artifact pack 완료
- [ ] Phase 3. Grok Runtime 격리·launch policy 완료
- [ ] Phase 4. Grok ACP supervisor·allowlist 완료
- [ ] Phase 5. Grok OAuth·model·session Gateway 완료
- [ ] Phase 6. Agent-aware Android client·UI 완료
- [ ] Phase 7. Grok stream·Stop·retry 정책 완료
...
- [ ] `AgentId(CODEX, GROK)`, capability, account, model, login, conversation, turn event 계약을 정의한다.
- [ ] conversation 저장 형식에 version과 `agentId`를 추가하고 기존 record는 `CODEX`로 안전 migration한다.
- [ ] AgentRouter가 selected Agent, active login, active turn을 단일 lock/state machine으로 소유하게 한다.
- [ ] Codex 기존 protocol/service를 `CodexAgentAdapter` 경계로 감싸되 method/payload를 변경하지 않는다.
- [ ] Agent switch precondition과 stable error code를 고정한다.
- [ ] health와 gateway readiness를 backend readiness에서 분리한다.
- [ ] public API에서 executable path, env, raw method를 입력받지 않게 한다.

### 자체 테스트

- [ ] 기존 Codex Python/Kotlin test가 변경 전과 동일하게 통과한다.
- [ ] old encrypted record가 Codex conversation으로 migration되고 원문 손실이 없다.
- [ ] Agent ID 누락·unknown·case 변형이 임의 backend 실행으로 이어지지 않는다.
- [ ] login/turn 중 switch가 거부되고 backend method가 호출되지 않는다.
- [ ] idle switch 실패가 prompt replay·자동 rollback을 만들지 않는다.
...
- [ ] 공통 계약 도입 뒤 Codex 기능과 fixture가 유지된다.
- [ ] Grok-specific protocol이 Android UI에 노출되지 않는다.
- [ ] 다음 Phase 진행 가능
...
- [ ] `grok-cli-pack` module과 `grok-cli.lock.json`을 추가한다.
- [ ] lock에 version `1.0.0`, source URL, `133745832` bytes, 고정 SHA-256과 source revisions를 기록한다.
- [ ] local path→외부 Gradle cache→고정 official URL 순서의 prepare task를 구현한다.
- [ ] upstream URL 사용 시 HTTPS·exact filename·expected size·SHA-256을 모두 검증한다.
- [ ] ELF AArch64, dynamic interpreter 부재, `grok --version` 출력을 검증한다.
- [ ] generated debug asset만 APK에 포함하고 release source set/task는 만들지 않는다.
- [ ] app-private staging은 temp copy→fsync 가능 범위→mode→atomic rename→rehash 순으로 구현한다.
- [ ] 기존 hash mismatch binary는 실행하지 않고 stable artifact error를 반환한다.
- [ ] 공식 license/notice와 upstream checksum 미제공 잔여 위험을 문서화한다.

### 자체 테스트

- [ ] wrong version/size/SHA/ELF/ABI가 각각 build 또는 staging에서 실패한다.
- [ ] offline cache hit build가 network 없이 재현된다.
- [ ] binary가 Git index/history에 없고 generated output이 ignore된다.
- [ ] debug APK의 Grok asset hash가 lock과 일치한다.
- [ ] release signing/store key/release artifact가 생성되지 않는다.
...
- [ ] pinned Grok artifact 공급과 fail-closed 검증이 자동화됐다.
- [ ] SBOM/notice/source map이 갱신됐다.
- [ ] 다음 Phase 진행 가능
...
- [ ] Agent runtime path object에 Grok home/staging/profile guest·host path를 추가한다.
- [ ] directory creation 시 canonical app-private path, symlink 부재, mode `0700`을 검증한다.
- [ ] process `umask 077`과 생성 credential/config mode `0600` 검증 probe를 구현한다.
- [ ] 고정 launch command와 환경 allowlist builder를 구현한다.
- [ ] API key·logging·auto-update·subagent 관련 금지 env를 검사한다.
- [ ] project-owned chat-only Agent profile을 hash-locked template로 stage한다.
- [ ] profile에서 mutating tool·subagent·MCP가 실제 initialize/session capability에 노출되지 않는지 검증한다.
- [ ] selected Agent process만 실행하고 switch/stop 시 bounded shutdown한다.

### 자체 테스트

- [ ] Codex와 Grok HOME/config/session 경로가 겹치지 않는다.
- [ ] wrong permission·symlink·profile hash mismatch에서 process가 시작되지 않는다.
- [ ] environment snapshot에는 allowlist key만 있고 secret/API key가 없다.
- [ ] `--no-auto-update`, telemetry off, subagent off가 fixture와 real credential-free smoke에서 확인된다.
- [ ] Samsung app-private Alpine에서 version과 ACP initialize smoke가 재현된다.
- [ ] Runtime stop 뒤 Grok/PRoot child process가 남지 않는다.
...
- [ ] Grok process 경계·권한·환경이 Codex와 독립적으로 고정됐다.
- [ ] credential 없는 Samsung initialize smoke가 통과했다.
- [ ] 다음 Phase 진행 가능
...
- [ ] Grok ACP process lifecycle `STOPPED→STARTING→INITIALIZING→READY→STOPPING/FAILED`를 구현한다.
- [ ] stdin write serialization, monotonic request ID, max pending request, method별 timeout을 구현한다.
- [ ] stdout strict UTF-8·object-only·1 MiB line limit·unknown response handling을 적용한다.
- [ ] stderr는 OAuth/token/account/prompt 패턴을 폐기하는 bounded redaction만 유지한다.
- [ ] initialize response의 protocolVersion, `grok.com`, modelState, session capability를 bounded parse한다.
- [ ] 허용 ACP method wrapper만 public으로 노출하고 raw `request(method, params)`를 service 경계 밖에 노출하지 않는다.
- [ ] forbidden method 목록을 verifier와 test fixture에 고정한다.
- [ ] notification reorder·duplicate terminal·late response·process generation mismatch를 처리한다.

### 자체 테스트

- [ ] fragmented/combined/malformed/oversized JSON line을 검증한다.
- [ ] duplicate/unknown ID, timeout, max pending, process crash가 bounded error로 끝난다.
- [ ] `getApiKey`, `setApiKey`, `getBearerToken`, unknown `x.ai/*`가 write 함수에 도달하지 않는다.
- [ ] stderr chunk boundary를 가로지르는 token·URL·email 패턴이 저장되지 않는다.
- [ ] initialize fixture reorder에도 capability가 동일하게 normalize된다.
- [ ] shutdown이 active request를 완료 또는 취소하고 child process를 회수한다.
...
- [ ] Grok ACP transport와 allowlist가 fake fixture에서 완성됐다.
- [ ] 임의 ACP proxy/API key 경로가 없다.
- [ ] 다음 Phase 진행 가능
...
- [ ] `authenticate(grok.com)`을 전용 async task와 request sequence로 single-flight 실행한다.
- [ ] `x.ai/auth/get_url`을 병행 호출해 `mode=device` complete URL만 검증·반환한다.
- [ ] URL은 HTTPS, allowlisted xAI host, length·scheme·userinfo·fragment 정책을 통과해야 한다.
- [ ] Android/Gateway가 user code나 stderr를 파싱·저장하지 않게 한다.
- [ ] login status는 pending/authenticated/failed/cancelled/expired stable state만 노출한다.
- [ ] cancel은 current request sequence만 최대 1회 취소하고 late authenticate response를 폐기한다.
- [ ] auth info/logout 응답의 email/profile/token 필드를 즉시 폐기하고 Boolean만 반환한다.
- [ ] modelState와 `x.ai/models/list`를 bounded normalize하고 hardcoded fallback을 두지 않는다.
- [ ] session new/resume/model change/close를 Agent-tagged binding에 연결한다.
- [ ] Gateway normalized endpoint와 security middleware를 연결한다.

### 자체 테스트

- [ ] get_url-before-ready, authenticate-before-get_url, cancel race, late success를 재현한다.
- [ ] duplicate login click이 authenticate를 두 번 호출하지 않는다.
- [ ] invalid/oversized/non-HTTPS URL이 Android로 전달되지 않는다.
- [ ] account/logout 응답에 email·profile·token이 존재해도 외부 JSON과 log에 남지 않는다.
- [ ] API key disabled initialize에서 `grok.com`만 표시된다.
- [ ] 모델 0/1/다수/duplicate/removal/malformed catalog를 처리한다.
- [ ] session binding의 Agent/generation mismatch가 resume/prompt 전에 거부된다.
...
- [ ] fake ACP에서 Grok OAuth→model→session lifecycle이 통과한다.
- [ ] credential·API key·계정정보 비노출 gate가 통과한다.
- [ ] 다음 Phase 진행 가능
...
- [ ] Gateway client에 agents/select/account/login/model/chat/interrupt/logout typed API를 추가한다.
- [ ] ViewModel을 Agent-aware state로 migration하고 한 active login/turn invariant를 유지한다.
- [ ] 상단에는 compact Agent selector와 model selector만 두고 세부 상태·login/logout/runtime는 sheet/overflow에 둔다.
- [ ] application ID는 유지하되 debug app 표시 이름을 Codex 전용으로 오해하지 않는 중립 이름으로 변경한다.
- [ ] Agent switch confirmation은 unsent draft·active conversation 영향만 설명하고 자동 logout하지 않는다.
- [ ] Grok login은 complete URL을 ACTION_VIEW로 열고 URL/code를 Compose semantics·clipboard·saved state에 보관하지 않는다.
- [ ] 민감 인증 화면에 security plan의 FLAG_SECURE·recents·clipboard 정책을 적용한다.
- [ ] conversation store에 Agent ID를 저장하고 Agent별 목록·새 대화·resume을 분리한다.
- [ ] Generating/LoginPending/Switching 중 selector와 위험 action을 비활성화한다.
- [ ] stable error에는 재전송 없는 `상태 다시 확인`, `Agent 다시 시작` 같은 명시 action만 제공한다.

### 자체 테스트

- [ ] Agent 1/2개, 모델 0/1/다수에서 selector와 채팅 높이가 유지된다.
- [ ] login pending/streaming/switching 중 중복 click이 backend 호출을 늘리지 않는다.
- [ ] Codex/Grok conversation과 selected model이 섞이지 않는다.
- [ ] process recreation 뒤 prompt 자동 전송 없이 selected Agent와 축약 상태만 복구한다.
- [ ] OAuth URL/code가 saved state, semantics dump, screenshot evidence, clipboard에 남지 않는다.
- [ ] 기존 composer navigation inset, Stop, 대화 목록 instrumentation이 회귀하지 않는다.
...
- [ ] fake backend에서 Codex/Grok 선택→login→model→chat→Stop→logout 메뉴별 테스트가 통과한다.
- [ ] 채팅 영역 중심 GUI와 접근성 기준이 유지된다.
- [ ] 다음 Phase 진행 가능
...
- [ ] `session/prompt`를 user text 정확히 1회로 dispatch하고 request/session/generation을 결합한다.
- [ ] ACP content update를 `start→delta*→done/error→[DONE]`으로 normalize한다.
- [ ] terminal event 첫 1개만 채택하고 late/duplicate delta를 폐기한다.
- [ ] Stop을 current session/request의 `session/cancel` 최대 1회에 연결한다.
- [ ] client disconnect·Runtime stop·Agent switch cleanup의 cancel 우선순위를 고정한다.
- [ ] CLI retry notification을 관찰해 pre-output/post-output 상태와 시도 횟수를 redacted metric으로만 분류한다.
- [ ] Android/Gateway dispatch count를 항상 1로 검증한다.
- [ ] partial delta 뒤 retry/auth recovery는 stable failure로 종료하고 assistant node를 새로 만들지 않는다.
- [ ] G1 strict 선택 시 retry 발견 즉시 Grok turn을 차단하고 공식 binary limitation을 보고한다.

### 자체 테스트

- [ ] normal delta, empty delta, large delta, malformed update, duplicate terminal을 검증한다.
- [ ] Stop before start ACK, during delta, after done가 idempotent하다.
- [ ] 401 before output과 after partial output fixture에서 허용 정책과 dispatch count가 일치한다.
- [ ] CLI retry exhaustion이 prompt 자동 replay나 다른 Agent fallback을 만들지 않는다.
- [ ] user node 1개, assistant node 1개, terminal state 1개 판정이 유지된다.
- [ ] process crash와 timeout 뒤 사용자가 명시 재전송하기 전 추가 turn이 없다.
...
- [ ] 스트리밍·Stop·retry 경계가 G1 결정과 테스트로 고정됐다.
- [ ] Android/Gateway 자동 retry·replay·fallback이 0회다.
- [ ] 다음 Phase 진행 가능
