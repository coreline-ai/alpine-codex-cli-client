...
누락·symlink·경로 이탈을 거부하고 각 Alpine `.apk`의 signed member와 `.PKGINFO` name/version/arch를
lock과 대조한다. 런타임 `apk`가 기존 Alpine 키링으로 최종 패키지 서명을 확인하며
...
- 검토된 Alpine `aarch64` Python/전이 패키지 팩을 로컬 빌드 입력으로 제공
- 서명된 APK/AAB에 `verify-release-artifact.py` 실행
- Phase 8 별도 UID broker와 Phase 9 전체 회귀·Samsung 인수 기준 완료
- 스토어 제출은 사용자 승인 후 별도 수행
