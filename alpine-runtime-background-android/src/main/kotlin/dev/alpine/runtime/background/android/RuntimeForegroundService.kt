...
class RuntimeForegroundService : Service() {
    ...
    override fun onCreate() {
        ...
        manager.createNotificationChannel(
            NotificationChannel(
                CHANNEL_ID,
                "Alpine Codex Runtime",
                NotificationManager.IMPORTANCE_LOW,
            ).apply {
                description = "사용자가 시작한 Alpine Codex Gateway 실행 상태"
                setShowBadge(false)
            },
        )
    }
    ...
    private fun createNotification(): Notification {
        ...
        return Notification.Builder(this, CHANNEL_ID)
            ...
            .setContentTitle("Alpine Codex Runtime 실행 중")
            .setContentText("Codex CLI Gateway가 실행 중입니다.")
            .setCategory(Notification.CATEGORY_SERVICE)
            .setOngoing(true)
            .setOnlyAlertOnce(true)
            .addAction(Notification.Action.Builder(null, "Runtime 중지", stopPendingIntent).build())
            .build()
    }

    companion object {
        internal const val ACTION_START = "dev.alpine.codexclient.runtime.START"
        internal const val ACTION_STOP = "dev.alpine.codexclient.runtime.STOP"
        internal const val ACTION_STOP_STALE = "dev.alpine.codexclient.runtime.STOP_STALE"
        private const val CHANNEL_ID = "alpine-codex-runtime"
        internal const val NOTIFICATION_ID = 0xC0DE
        private const val STOP_REQUEST_CODE = 0xC0DE
    }
}
